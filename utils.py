...
def plot_generated_data(title: str, matrix_betas: numpy.ndarray, results_dir: str):
    ...
    seaborn.heatmap(matrix_betas, center=0, vmin=matrix_betas.min(), vmax=matrix_betas.max(), cmap='coolwarm', ax=ax0,cbar_kws={'shrink': 0.5})
    ...
    seaborn.heatmap(numpy.mean(matrix_betas, 0).reshape(1, -1), center=0, vmin=numpy.mean(matrix_betas, 0).min(), vmax=numpy.mean(matrix_betas, 0).max(), cmap='coolwarm', square=True, ax=ax1, cbar_kws={'shrink': 0.3})
    ax1.set_ylabel("mean")
    ...
    seaborn.heatmap(numpy.var(matrix_betas, 0).reshape(1, -1), vmin=0, vmax=numpy.var(matrix_betas, 0).max(), cmap='Reds', square=True, ax=ax2, cbar_kws={'shrink': 0.3})
    ax2.set_ylabel("var")
    ...
    seaborn.heatmap(numpy.corrcoef(matrix_betas), vmin=0, vmax=1, cmap='Reds', square=True, ax=ax5,cbar_kws={'shrink': 0.3})
    ax5.title.set_text("Q0 matrix ({})".format(numpy.round(numpy.corrcoef(matrix_betas).mean(), 2)))
    ...
    seaborn.heatmap(numpy.corrcoef(matrix_betas.T), vmin=0, vmax=1, cmap='Reds', square=True, ax=ax6,cbar_kws={'shrink': 0.3})
    ax6.title.set_text("Spatial correlation matrix ({})".format(numpy.round(numpy.corrcoef(matrix_betas.T).mean(), 2)))
    ax6.set_ylabel("J Voxels")
    ax6.set_xlabel('J Voxels')

    plt.suptitle('{} : information'.format(title))
    plt.tight_layout()
...
def plot_results(title: str, matrix_betas: numpy.ndarray, deltas: numpy.ndarray, deltas_var: numpy.ndarray, T_map: numpy.ndarray, W: numpy.ndarray, results_dir: str):
    ...
    # display generated data
    seaborn.heatmap(matrix_betas, center=0, vmin=-3, vmax=3, cmap='coolwarm', ax=axs[0],cbar_kws={'shrink': 0.5})
    ...
    seaborn.heatmap(matrix_betas.mean(axis=0).reshape(1, -1), center=0, vmin=deltas.min(), vmax=deltas.max(), cmap='coolwarm', square=True, ax=axs[1],cbar_kws={'shrink': 0.3})
    axs[1].set_ylabel("Mean")
 
    # display Deltas and Deltas variance
    seaborn.heatmap(deltas.reshape(1, -1), center=0, vmax=deltas.max(), vmin=deltas.min(), cmap='coolwarm', square=True, ax=axs[2], cbar_kws={'shrink': 0.3})
    axs[2].set_ylabel("Deltas")
    seaborn.heatmap(deltas_var.reshape(1, -1), center=0, vmax=deltas_var.max(), vmin=0, cmap='coolwarm', square=True, ax=axs[3], cbar_kws={'shrink': 0.3})
    axs[3].set_ylabel("Variance deltas")


    # display T map
    seaborn.heatmap(T_map.reshape(1, -1), center=0, vmin=T_map.min(), vmax=T_map.max(), cmap='coolwarm', square=True, ax=axs[4], cbar_kws={'shrink': 0.3})
    axs[4].set_ylabel("T stat")

    # display p_values
    p_values = 1 - scipy.stats.norm.cdf(T_map)
    seaborn.heatmap(p_values.reshape(1, -1), vmin=0, vmax=0.06, cmap='Reds_r', square=True, mask=p_values.reshape(1, -1) > 0.05, ax=axs[5], cbar_kws={'shrink': 0.3})
    axs[5].title.set_text("significant p values")
...
def display_matrices(results_dir, sim_number, corr):
    ...
    for ind, matrice in enumerate(correlation_matrices):
        ...
        if ind < 4:
            ...
            row = ind - 4
            col = 1
        seaborn.heatmap(matrix, center=0, vmax=0.5, vmin=-0.5, cmap='coolwarm', robust=True, square=True, ax=axs[row, col], cbar_kws={'shrink': 0.6})
        seaborn.heatmap(matrix_organized, center=0, vmax=0.5, vmin=-0.5, cmap='coolwarm', robust=True, square=True, ax=axs[row, col+2], cbar_kws={'shrink': 0.6})
        seaborn.heatmap(matrix_organized_louvain, center=0, vmax=0.5, vmin=-0.5, cmap='coolwarm', robust=True, square=True, ax=axs[row, col+4], cbar_kws={'shrink': 0.6})
